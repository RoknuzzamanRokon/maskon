...
function EditPortfolioContent() {
  ...
  const fetchPortfolioItem = async () => {
    try {
      ...
      const item = items.find((p: any) => p.id === portfolioId);

      if (!item) {
        setMessage("Portfolio item not found");
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updatePortfolioItem(portfolioId, cleanedData);
      ...
      setMessage("Error updating portfolio item. Please try again.");
    } finally {
