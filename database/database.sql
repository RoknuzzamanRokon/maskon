-- ============================================================
-- COMPLETE DATABASE SCHEMA FOR BLOG & PORTFOLIO WEBSITE
-- ============================================================
-- Features:
-- - Blog posts (tech, food, activity)
-- - Portfolio projects
-- - User authentication & admin system
-- - Post interactions (likes, dislikes, comments)
-- - Anonymous interactions
-- - Products catalog with e-commerce
...
DELIMITER //
CREATE TRIGGER IF NOT EXISTS update_session_status_on_admin_response
AFTER INSERT ON product_chat_messages
FOR EACH ROW
BEGIN
...
SELECT 
    ...
    COUNT(CASE WHEN cm.is_read = FALSE AND cm.sender_type = 'customer' THEN 1 END) as unread_customer_messages,
    COUNT(CASE WHEN cm.is_read = FALSE AND cm.sender_type = 'admin' THEN 1 END) as unread_admin_messages
...
-- DELIMITER //
CREATE PROCEDURE IF NOT EXISTS CreateChatSession(
    IN p_product_id INT,
...
BEGIN
    ...
    INSERT INTO product_chat_sessions (
        ...
        p_product_id, p_session_id, p_customer_email, p_customer_name, 'pending', 'medium'
    );
    ...
    IF p_initial_message IS NOT NULL AND p_initial_message != '' THEN
        INSERT INTO product_chat_messages (
            ...
            session_pk, 'customer', p_customer_name, p_initial_message, 'text'
        );
    END IF;
...
DELIMITER //
CREATE PROCEDURE IF NOT EXISTS SendChatMessage(
...
BEGIN
    ...
    INSERT INTO product_chat_messages (
        ...
        session_pk, p_sender_type, p_sender_id, p_sender_name, p_message_text, COALESCE(p_message_type, 'text')
    );
...
INSERT INTO portfolio (title, description, technologies, project_url, github_url, image_url)
VALUES 
    (
        'E-commerce Website',
...
VALUES 
    ...
    ('accessories', 'Various accessories and add-ons', TRUE),
    ('home', 'Home and living products', TRUE)
ON DUPLICATE KEY UPDATE name = name;

-- Insert sample products
INSERT INTO products (name, description, category, price, stock, discount, specifications, image_url, is_active, created_by)
VALUES 
    (
